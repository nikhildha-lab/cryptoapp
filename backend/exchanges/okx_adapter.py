...
class OKXAdapter(ExchangeAdapter):
    ...
    def __init__(self, api_key=None, api_secret=None, is_paper=True):
        super().__init__("okx", api_key, api_secret, is_paper)
        
        options = {
            'apiKey': api_key,
            'secret': api_secret,
            # OKX often requires a password/passphrase
        ...
        if is_paper and api_key and api_secret:
            ...
            self.logger.info("Initialized OKX in SANDBOX mode")
        else:
            self.logger.info("Initialized OKX in LIVE mode (Simulation may apply if no keys)")
